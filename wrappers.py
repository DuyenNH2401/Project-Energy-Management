...
# 9-16: surroundings (LF_d, LF_v, LB_d, LB_v, RF_d, RF_v, RB_d, RB_v)
IDX_L_DIST      = 17
IDX_L_REL_SPEED = 18
IDX_SPEED_LIMIT = 19
IDX_CAN_LEFT    = 20
...
class FullControlWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if not terminated or info.get("is_success", 0):
            ...
            # Progress (velocity already normalized)
            r_progress = obs[IDX_VELOCITY] * (cfg.MAX_SPEED / 15.0)
            ...
            # Smooth steer
            r_smooth_steer = abs(steer - self.prev_steer)
            ...
            # Energy efficiency (Wh/m) — CRITICAL for research validity.
            # KD student must retain energy knowledge after annealing (α=β=0).
            r_efficiency = compute_energy_efficiency(obs)

            # TLS violation: penalty for approaching red light at high speed
            tls_state = obs[IDX_TLS_STATE]
            tls_dist = obs[IDX_TLS_DIST]
            speed_ratio = obs[IDX_VELOCITY]  # already normalized
            r_tls = 0.0
            ...
            reward += (
                self.W.get("progress", 0.0)           * r_progress +
                self.W.get("alive", 0.0)              * r_alive +
                self.W.get("speed_target", 0.0)       * r_speed_target +
                self.W.get("too_slow", 0.0)           * r_too_slow +
                self.W.get("smooth_steer", 0.0)       * r_smooth_steer +
                self.W.get("smooth_accel", 0.0)       * r_smooth_accel +
                self.W.get("safety_leader", 0.0)      * r_leader +
                self.W.get("energy_efficiency", 0.0)  * r_efficiency +
                self.W.get("tls_violation", 0.0)      * r_tls
            )
